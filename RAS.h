...
#ifndef __RAL_RAS__RAS__
#define __RAL_RAS__RAS__
...
void performUpilson(char* alignFile, char* topFile, int minNumRateCat, int maxNumRateCat,
				int numCpuThreads, int randomInit, int numIterations, string prefixOut, char* preLogFile);
 */
...
// The data structure for the function RASIter used in multi-threaded environment
typedef struct RASIterArgs {
	int num_chars;
	Alignment* alignment;
	int* topMatrix;
	int numLineTopMat;
...
#endif /* defined(__RAL_RAS__RAS__) */
